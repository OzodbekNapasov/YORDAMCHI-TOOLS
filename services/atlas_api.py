...
# ============================================================
# 8. PC CONTROL & SYSTEM AGENT REST API ENDPOINTS (REALTIME BRIDGE)
# ============================================================

@atlas_api.route("/pc/status", methods=["GET"])
@admin_required
def api_pc_status():
    """Kompyuter tizim holati (Realtime Cloud-to-Local Bridge orqali)"""
    try:
        from services.pc_control.bridge import get_bridge_pc_status
        data = get_bridge_pc_status()
        return jsonify({
            "success": True,
            "data": data
        })
...
def api_pc_webcam():
    ...
    try:
        from services.pc_control.bridge import dispatch_bridge_command
        res = dispatch_bridge_command("webcam", timeout=8.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_power():
    ...
    try:
        from services.pc_control.bridge import dispatch_bridge_command
        data = request.get_json(silent=True) or {}
        res = dispatch_bridge_command("power", data, timeout=6.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_apps():
    ...
    try:
        ...
        status = get_bridge_pc_status()
        if status.get("apps"):
            return jsonify({"success": True, "apps": status["apps"]})
        res = dispatch_bridge_command("apps", timeout=5.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_kill():
    ...
    try:
        ...
        res = dispatch_bridge_command("kill", {"target": target}, timeout=6.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_cmd():
    ...
    try:
        ...
        res = dispatch_bridge_command("cmd", {"command": cmd}, timeout=8.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_cleanup():
    ...
    try:
        from services.pc_control.bridge import dispatch_bridge_command
        data = request.get_json(silent=True) or {}
        res = dispatch_bridge_command("cleanup", data, timeout=6.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_media():
    ...
    try:
        from services.pc_control.bridge import dispatch_bridge_command
        data = request.get_json(silent=True) or {}
        res = dispatch_bridge_command("media", data, timeout=5.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_pc_sunshine():
    ...
    try:
        ...
        res = dispatch_bridge_command("sunshine", {"pin": pin}, timeout=6.0)
        return jsonify(res)
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
